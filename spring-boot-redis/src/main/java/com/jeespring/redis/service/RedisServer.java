...
public class RedisServer {
    ...
    public long incr(String key, long by) {
        if (by < 0) {
            throw new RuntimeException("By must be greater than 0 !");
        }
     ...
     * @return true成功 false失败
     */
    public <T> boolean hmSet(String key, Map<String, T> map, long timeout) {
        try {
            redisTemplate.opsForHash().putAll(key, map);
            if (timeout > 0) {
                expire(key, timeout);
     ...
     * @return true 成功 false失败
     */
    public <T> boolean hSet(String key, String item, T value, long timeout) {
        try {
            redisTemplate.opsForHash().put(key, item, value);
            if (timeout > 0) {
                expire(key, timeout);
     ...
     * @return
     */
    public <T> boolean lSet(String key, T value, long timeout) {
        try {
            redisTemplate.opsForList().rightPush(key, value);
            if (timeout > 0) {
                expire(key, timeout);
    ...
    public boolean lSet(String key, List<Object> value, long timeout) {
        try {
            redisTemplate.opsForList().rightPushAll(key, value);
            if (timeout > 0) expire(key, timeout);
            return true;
        } catch (Exception e) {
            log.error("lSet error", e);
            return false;
